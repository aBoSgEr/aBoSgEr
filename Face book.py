...
def password():
  password='0'
  passw=input(f'{B} Enter Password{S} :{X} ')
  if passw == password:
    print(f'{F}Good Password ')
    time.sleep(0.8)
  else:
    print(f'{R}Error Password ')
    time.sleep(0.8)
    password()
password()
os.system('clear')
print('')
print(('—'*25)+'\n•ﺮــﻜـﺻ ﻮـﺑﺍ ﺕﺍﻭﺩﻻﺍ ﻕﻼـﻤﻋ•\n'+('—'*25))
...
tlg1 = '\n ⋯⋯⋯⋯⋯⋯⋯⧏ BrHoM ⧐⋯⋯⋯⋯⋯⋯⋯\n𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃  ➩ \nتم تشغيل الاداة\n    \nلا تـنسى ان تـرسل صـور الـصيد -  @T_awi\n\n⋯⋯⋯⋯⋯⋯⋯⧏ BrHoM ⧐⋯⋯⋯⋯⋯⋯⋯ '
...
def main():
    ...
    gh = 'h'
    print('—'*25)
    print('• ﺮــﻜـﺻ ﻮـﺑﺍ ﺕﺍﻭﺩﻻﺍ ﻕﻼـﻤﻋ •')
    print('—'*25)
    ...
    print('      ﻱﺪﻳﻻﺍ ﻦﻣ ﻞﻀﻓﺍ 1 ﻢﻗﺭ ﺭﺎـﺘﺧﺍ     ')
    print(f'''{N}–{O}–{M}–{B}–{Z}–{m}–{x}–{m}–{H}–{u}–{P}–{b}–{P}–{kk}–{k}–{hh}–{O}–{M}–{B}–{Z}–{m}–{x}–{m}–{H}–{u}–{P}–{b}–{m}–{H}–{u}–{P}–{b}–{P}''')
...
def crack(idf, pwv):
    ...
    fff = '%'
    print('\r%s [AboSgAeR] %s/%s - OK : %s - CP : %s - %s%s%s' % (bi, loop, len(id2), ok, cp, int(pers), str(fff), x), end=' ')
    sys.stdout.flush()
